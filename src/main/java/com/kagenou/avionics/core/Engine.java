...
public final class  Engine {
    ...
    private float ctx, cty, ctz; // camera target (orbit follow)

    // Explicit camera (first-person): when set, overrides the orbit follow for one
    // configuration until setCameraTarget() switches back.
    private boolean explicitCam;
    private float camEx, camEy, camEz;   // eye
    private float camCx, camCy, camCz;   // look-at center
    ...
    /** First-person: place the eye explicitly and look toward {@code center} (up = +Y). */
    public void setCameraLookAt(float eyeX, float eyeY, float eyeZ,
    ...
    public void beginFrame() {
        ...
        float[] eye, center;
        if (explicitCam) {
            eye = new float[]{camEx, camEy, camEz};
            center = new float[]{camCx, camCy, camCz};
        } else {
            center = new float[]{ctx, cty, ctz};
            eye = new float[]{ctx + EYE_OFFSET[0], cty + EYE_OFFSET[1], ctz + EYE_OFFSET[2]};
        }
