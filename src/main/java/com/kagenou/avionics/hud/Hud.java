...
public final class Hud {
    ...
    /** Minimal top-left overlay for the PS5 flight mode (FPS / camera / pathing / position). */
    public void drawPs5Hud(int w, int h, double fps, String modeName, String bannerText,
                           float[] bannerColor, String camMode, boolean recording,
                           float x, float y, float z, Controls c) {
        beginOrtho(w, h);
        drawStatusBanner(w, modeName, bannerText, bannerColor, c.paused);

        float tx = 12, ty = 50, lh = 18, s = 1.5f;
        text.draw(tx, ty, s, Palette.TEXT, 1f, String.format("FPS: %.0f", fps));
        ty += lh;
        text.draw(tx, ty, s, Palette.TEXT, 1f, "Cam: " + camMode);
        ty += lh;
        text.draw(tx, ty, s, recording ? Palette.ACCENT_GREEN : Palette.TEXT_DIM, 1f,
                "Pathing: " + (recording ? "REC" : "OFF"));
        ty += lh;
        text.draw(tx, ty, s, Palette.TEXT, 1f, String.format("Pos (Q8.8): (%.2f, %.2f, %.2f)", x, y, z));
